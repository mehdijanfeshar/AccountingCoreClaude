...
- **2026-09-23** (فاز ۴۲، هر دو ریپو): **«دسترسی کدینگ حسابداری»** — تعیین اینکه هر معین را کدام **نوع واحدها** می‌توانند استفاده کنند (`TB_WHITEANDBLACKLIST`): ساخت دسته‌ای دکارتی + غیرفعال‌سازی/فعال‌سازی مجدد + ۷ فیلتر سمت سرور + `GET /api/vahed-types` + صفحهٔ فرانت با دو درخت چک‌باکسی. کوئری روی اوراکل زنده سه چیز را قطعی کرد: بُعد «نوع واحد» است نه واحد، جفت (معین، نوع واحد) **یکتا نیست** (۲۹۱ جفت چندردیفی)، و ۲۸ از ۶۰ معین هیچ ردیف مجازی ندارند. ⚠️ **ماتریس نوشته می‌شود ولی هنوز اعمال نمی‌شود** — تصمیم صریح صاحب پروژه (ریسک #۲۷). **۲۹۶۰ تست سبز** (از ۲۸۷۶). جزئیات: `docs/phase-log.md` بخش «فاز ۴۲».

- **2026-09-22**: **فازهای ۳۹ تا ۴۱** — سه عملیات کارتابل (معکوس سند، مرتب‌سازی، تغییر وضعیت) + «تأیید دائم» پایانی + رفع باگ نمایش تاریخ در همهٔ فرم‌ها؛ گزارش ماتریسی روی View اوراکل؛ و مرور اسناد + دفتر روزنامه + پیمایش کل↔جزء گزارش ماتریسی. ⚠️ **کشف شد که دو View آمادهٔ مرور اسناد و دفتر روزنامه از نظر عددی غلط‌اند** (ریسک #۲۶) و هر دو گزارش از مدل نوشتن خوانده می‌شوند — استثنای ثبت‌شدهٔ دوم بر قانون کاری #۲. **۲۸۷۶ تست سبز** (از ۲۷۵۰). جزئیات: `docs/phase-log.md` بخش‌های «فاز ۳۹»، «فاز ۴۰»، «فاز ۴۱».
