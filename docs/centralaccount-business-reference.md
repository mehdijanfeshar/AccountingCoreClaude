...
- **Create یک عملیات bulk کارتزین است:** `AccountCodeIds × VahedTypeIds` → N ردیف در یک Command. ~~ما تک‌تک می‌سازیم.~~ ✅ **در فاز ۴۲ (۲۰۲۶-۰۹-۲۳) پورت شد** — `CreateWhiteAndBlackListsBulkCommand`، در یک تراکنش.
- `ChangeStateToBlackListed` علاوه بر ست‌کردن `State = Blacklisted`، **هر چهار فیلد تاریخ را `null` می‌کند** (`FROMAUTHORIZEDDATE`, `TOAUTHORIZEDDATE`, `FROMLIMITATIONDATE`, `TOLIMITATIONDATE`). یک قانون کسب‌وکاری واقعی که ~~ما نداریم~~ ✅ **در فاز ۴۲ پورت شد** (`POST {id}/blacklist`، بدون بدنه — پاک‌کردن تاریخ‌ها بخشی از گذار است نه انتخاب فراخوان).
- `ReActive` هر چهار تاریخ + `State` را با هم می‌گیرد. ✅ در فاز ۴۲ پورت شد، ولی **عمداً با یک جفت تاریخ نه چهار** — رجوع به بند بعد.

#### 🔑 معنای `StateEnum` — تأییدشده از زبان صاحب پروژه (۲۰۲۶-۰۹-۲۳)

این را **حدس نزن و از روی نام ستون بازنساز نکن.** در فاز ۴۲ مستقیماً از صاحب پروژه پرسیده و پاسخ ثبت شد:

| مقدار | معنا |
|---|---|
| `Allowed = 1` | مجاز به ثبت دستی و سیستمی |
| `SystemOnly = 2` | **کاربر نمی‌تواند به‌صورت دستی از آن معین آرتیکل بزند؛ فقط سند سیستمی (مانند رابط‌های افتتاحیه/اختتامیه) اجازهٔ ثبت دارد.** |
| `Blacklisted = 3` | به‌طور کلی غیرمجاز |

برچسب رادیوی فیگمای همین صفحه («محدود کردن (ثبت سیستمی)») مستقلاً همین را تأیید می‌کند.

**قاعدهٔ ظریفی که از فیگما و Angular قدیمی هر دو استخراج شد:** جدول دو جفت ستون تاریخ دارد ولی فرم فقط **یک** بازه می‌گیرد، چون «کدام جفت پر شود» **خودِ معنای وضعیت است**: `Allowed` → جفت `AUTHORIZED`، `SystemOnly` → جفت `LIMITATION`، و جفت دیگر `null` می‌ماند. شاهد در گرید فیگما: ردیفی که تاریخ محدودیت دارد، تاریخ مجازش خالی است و برعکس. ما این routing را سمت سرور انجام می‌دهیم تا فراخوان نتواند ردیفی بسازد که هر دو جفتش پر باشد.

- **تأیید مجدد فاز ۱۳:** هیچ‌کدام از این جدول‌ها در هیچ مسیر نوشتن دیگری چک نمی‌شوند — ماتریس مجوزی است که نوشته می‌شود و هرگز اعمال نمی‌شود. ⚠️ **در فاز ۴۲ ما هم آگاهانه همین‌جا ایستادیم** (تصمیم صریح صاحب پروژه: «فعلاً فقط CRUD، اعمال بعداً») — ریسک #۲۷ در `CLAUDE.md`. یعنی این هشدار فعلاً دربارهٔ سیستم خودمان هم صادق است.

#### `TB_VAHED_TYPE.PARENTTYPECODE` — سطلی بدون lookup

`GetVahedTypeQueryTreeHandler` مرجع سطح میانی درخت را با `label = mainGroup.Key` می‌سازد، یعنی **کد خام** `PARENTTYPECODE` را بدون هیچ نام نمایشی برمی‌گرداند. در کل schema هم جدول lookup ای برای این سه سطل وجود ندارد. تنها سرنخ نام‌ها، کامنت خودِ تست مرجع است (`SetParentTypeCode("2"); // درمان`). نگاشت کاملی که ما استفاده می‌کنیم و **استنباط** است نه دادهٔ خوانده‌شده، در `docs/open-decisions.md` بخش فاز ۴۲ ثبت شده.
...
# ۲۵. «شناسه» در برابر «شناسنامه» — دو مکانیزم جدا با نام‌های شبیه (۲۰۲۶-۰۹-۲۰)

> **چرا این بخش وجود دارد:** نام جدول‌ها گمراه‌کننده است. خانوادهٔ `TB_IDENTITY*` به‌نظر می‌رسد پشتوانهٔ منوی «حسابهای شناسه دار» باشد — **نیست**. این اشتباه در ۲۰۲۶-۰۹-۲۰ نزدیک بود رخ دهد و با تصحیح صریح صاحب پروژه روشن شد. پیش از ساختن هر چیزی در این ناحیه، این بخش را بخوان.

## ۲۵-۱. جدول تمایز

| | **شناسه** | **شناسنامه** |
|---|---|---|
| جدول تعریف | `TB_ATTRIBFORACCOUNTCODE` | `TB_IDENTITYGROUP` + `TB_IDENTITYSUBGRP` |
| جدول مقدار (هنگام سند) | `TB_ATTRIBSINVOUCHER` | `TB_IDENTITYHEAD` + `TB_IDENTITYDETAIL` + `TB_IDENTITYFIXITEM` |
| اتصال به حساب | `ACCOUNTCODE_ID` (FK `FK_ATTRIBFO_ACCOUNTCODE`) | `TB_ACCOUNTCODE.IDENTYGROUPS_ID` |
| منوی پروژهٔ مرجع | «حسابهای شناسه دار» → `base-identity-account` | `base-feature/identity-head` + `identity-head-edit` |
| Endpoint پروژهٔ مرجع | `urlService.attribuite.getattributeSearch` | جدا |
| **وضعیت ما** | ✅ **کامل** — CRUD + لیست با ستون معین و پنل جستجو (۲۰۲۶-۰۹-۲۰) | ⚠️ **ناقص** — فقط `IdentityGroups` و `IdentitySubGroups` در بک‌اند؛ `Head`/`Detail`/`FixItem` هیچ Controller ای ندارند و فرانت هیچ صفحه‌ای ندارد |

## ۲۵-۲. شاهد قطعی

کامنت **هر پنج** جدول `TB_IDENTITY*` در schema اوراکل صریحاً واژهٔ **«شناسنامه»** را دارد:

- `TB_IDENTITYGROUP.ID` — «آي دي گروه اصلي **شناسنامه**»
- `TB_IDENTITYSUBGRP.ID` — «آي دي زير گروه **شناسنامه**»
- `TB_IDENTITYHEAD.SERIAL` — «سريال هر موضوع **شناسنامه**»
- `TB_IDENTITYDETAIL.DETAIL_VALUE` — «مقدار **شناسنامه**» (مقادیر **متغیر**)
- `TB_IDENTITYFIXITEM.FIXITEMS_VALUE` — «مقدار **شناسنامه**» (مقادیر **ثابت**)

⚠️ برخلاف قاعدهٔ §۱۰-۴ که کامنت‌های ستون این schema را غیرقابل‌اعتماد می‌داند، اینجا کامنت‌ها **یکدست و هم‌راستا با ساختار جدول‌ها** هستند و با تأیید صریح صاحب پروژه هم می‌خوانند — پس قابل اتکا.

## ۲۵-۳. آنچه از ساختار برمی‌آید (استنتاج، نه تأیید)

`TB_IDENTITYDETAIL` هم `IDENTITYSUBGRPS_ID` دارد هم **`VOUCHERSDETAIL_ID`** — یعنی مقدار هر زیرگروه شناسنامه به یک **ردیف سند** می‌چسبد. تفکیک `DETAIL` (متغیر) از `FIXITEM` (ثابت) یعنی بخشی از شناسنامه یک‌بار روی `IDENTITYHEAD` ثبت می‌شود و بخشی به‌ازای هر آرتیکل.

⚠️ این استنتاج از **ساختار** است. جریان کسب‌وکار واقعی‌اش در پروژهٔ مرجع (`base-feature/identity-head`) خوانده **نشده** — اگر روزی این بخش ساخته شود باید اول آن را خواند، نه از روی همین بخش حدس زد.
