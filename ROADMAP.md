...
| ۱۰. فرم صدور سند با تفصیلی داینامیک (Frontend) | ⬜ متوقف |
...
2. **🔴 حذف گرهٔ کدینگ vs فرزندان/ارجاعات** — تنها بخش باقی‌ماندهٔ ریسک «یکپارچگی حذف». ✅ سمت سند در فاز ۹ کاملاً حل شد (هر سه سطح)، ولی `DeleteAccountCodeCommand` همچنان بررسی نمی‌کند که گره فرزند دارد یا در ردیف‌های سند استفاده شده — بی‌صدا ناسازگاری تولید می‌کند.
