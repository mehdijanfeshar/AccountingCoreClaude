...
| ۱۲ | **«الزامی بودن تفصیلی» سمت سرور اعتبارسنجی نمی‌شود.** ⚠️ فاز ۳۴ فقط **نیمهٔ پیکربندی** را حل کرد: `TB_ACCOUNT_LINK_LEVEL` حالا قابل‌نوشتن است و با `TB_ACCOUNT_LINK_TAFSILGROUP` همگام می‌ماند، پس «کدام سطوح اجباری‌اند» بالاخره معنای درستی دارد. ولی **هیچ‌چیز در Application مانع ثبت ردیف سندی نمی‌شود که سطح اجباری‌اش را پر نکرده باشد** — فقط UI اجبارش می‌کند، که نقض مستقیم قاعدهٔ کاری #۱ است. پروژهٔ مرجع این بررسی را در سه مسیر مستقل دارد (سند مرجع §۳-۳). | باز از ابتدا؛ نیمه‌حل در ۳۴ |
