...
- [ ] رفع باگ‌های `bool?`→enum — **بخش عمده انجام شد.** اصلاح‌شده تا امروز: `TB_TAFSIL_LINK_TAFSILGROUP.VAHEDTYPE` (فاز ۲۱)، ۴ ستون `TB_ACCOUNTCODE` (فاز ۲۵)، و **۱۴ ستون روی ۸ Entity در فاز ۲۷**. **باقی‌مانده: ۸ ستون (دسته‌های ۴ و ۵ که عمداً انجام نشدند) + `TB_VOUCHERSHEAD.DOCLIFE`** — فهرست دقیق و آمادهٔ اجرا در `docs/open-decisions.md` بخش «فاز ۲۷». جدول مرجع: `docs/centralaccount-business-reference.md` §۲۴-۱ (⚠️ یک خطای تأییدشده دارد — `TB_CHECK.EBTAL`؛ رجوع به همان بخش `open-decisions.md`).
...
| # | ریسک 🔴 | کشف‌شده در |
|---|---|---|
| ۱ | **IDOR — نیمه بسته شد (فاز ۱۹).** ✅ لیست/جستجو/ساخت: `VahedCode` حالا سمت سرور از `ICurrentUser` تحمیل می‌شود (`VahedScopeBehavior`). 🔴 **همچنان باز:** `GetById`/`Update`/`Delete` هیچ بررسی مالکیت رکورد ندارند — با دانستن `id`، رکورد هر واحد دیگری قابل خواندن/ویرایش/حذف است. **تصمیم آگاهانهٔ صاحب پروژه، نه سهل‌انگاری.** ضمناً `PersonAction` هنوز کاملاً بی‌محافظ است (منتظر تصمیم کاربر). **مسدودکنندهٔ استقرار.** | فاز ۷–۱۶؛ نیمه‌حل در ۱۹ |
...
| ۱-ج | **نوشتن بین‌واحدی از طریق `CreateVoucherDetail`** — جست‌وجوی سرسند والد scope نمی‌خورد، پس می‌توان ردیف سند را به سرسند واحد دیگری چسباند (با دانستن `Id`). یک *نوشتن* است نه فقط خواندن. | فاز ۱۹ (Gate امنیتی) |
| ۱-الف | **`ValidationBehavior` هرگز برای هیچ `Update`/`Delete` اجرا نشده** — قید `where TRequest : IRequest<TResponse>` با `IRequest` غیرجنریک MediatR 14 برآورده نمی‌شود و DI بی‌صدا رد می‌کند. یعنی از فاز ۸ تا امروز FluentValidation روی ورودی‌های Update/Delete **بی‌اثر** بوده. ⚠️ `VahedScopeBehavior` عمداً این قید را ندارد؛ «تمیزکاری»‌اش IDOR را بازمی‌گرداند. | فاز ۱۹ |
| ۲ | **باگ `bool?`→enum روی ستون‌های چندمقداری `NUMBER(1)`** — از فاز ۱۷ فهرست **قطعی و کامل** است: **۱۶ ستون تأییدشده** (نه «مشکوک») به‌علاوهٔ `TYPECODE`/`DOCLIFE`. ⚠️ **فرض «اگر دومقداری باشد `bool` بی‌خطر است» باطل شد** — تقریباً همهٔ این enumها از **۱** شروع می‌شوند نه ۰. جدول کامل با مقادیر: `docs/centralaccount-business-reference.md` §۲۴-۱. **پیشرفت:** `VAHEDTYPE` لینک (فاز ۲۱)، ۴ ستون `TB_ACCOUNTCODE` (فاز ۲۵) و **۱۴ ستون روی ۸ Entity (فاز ۲۷)** اصلاح شدند. **باقی‌مانده: ۸ ستون (دسته‌های ۴/۵، عمداً انجام‌نشده) + `TB_VOUCHERSHEAD.DOCLIFE`** — فهرست دقیق: `docs/open-decisions.md` بخش «فاز ۲۷». هنگام اصلاح بقیه حتماً یافتهٔ فاز ۲۵ را اعمال کن: روی Oracle، enum بدون `.HasConversion<int?>()` صریح در predicate می‌ترکد (و `.HasColumnType("NUMBER(1)")` باید حذف شود؛ برای `short` دقیقاً برعکس). | فاز ۱۲–۱۷ |
