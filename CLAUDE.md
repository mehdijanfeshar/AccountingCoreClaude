...
- [ ] رفع باگ‌های `bool?`→enum — فهرست انباشته حالا **~۱۲ ستون در مسیر نوشتن فعال** است (از فازهای ۱۲/۱۳/۱۴). جزئیات: `docs/open-decisions.md`.
...
| # | ریسک 🔴 | کشف‌شده در |
|---|---|---|
| ۱ | **IDOR — هیچ authorization در سطح رکورد وجود ندارد.** هر کاربر احراز‌شده می‌تواند هر رکوردی از هر واحد سازمانی را بخواند/بسازد/ویرایش/حذف کند؛ `VAHEDCODE` سمت سرور اعمال نمی‌شود. با هر فاز سطح حمله بزرگ‌تر شده (حالا همهٔ Endpointهای فازهای ۷–۱۴). **مسدودکنندهٔ استقرار.** | فاز ۷/۸/۱۰/۱۳/۱۴ |
| ۲ | **باگ `bool?`→enum روی ستون‌های چندمقداری `NUMBER(1)`** — ~۱۲ ستون در مسیر نوشتن فعال، شامل موارد تأییدشدهٔ `TB_ACCOUNTCODE.TYPECODE`، `TB_VOUCHERSHEAD.DOCLIFE`، `TB_WHITEANDBLACKLIST.STATE`. | فاز ۱۲/۱۳/۱۴ |
