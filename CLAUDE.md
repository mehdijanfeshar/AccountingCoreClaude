...
| # | ریسک 🔴 | کشف‌شده در |
|---|---|---|
| ۱ | **IDOR — در حال بسته‌شدن (فاز ۱۹ + فاز ۳۲ جاری).** ✅ لیست/جستجو/ساخت از فاز ۱۹ سمت سرور scope می‌شوند. 🔶 **فاز ۳۲ (۲۰۲۶-۰۹-۲۱) مکانیزم بررسی مالکیت رکورد را ساخت و اولین Entity را مهاجرت داد:** امضای `GetForUpdateAsync`/`GetByIdAsync` حالا `vahedCode` **الزامی** می‌گیرد (کامپایلر هر call site را مجبور می‌کند)، `VahedOwnership` تنها جای تصمیم است، و رکورد واحد دیگر → `UnitAccessDeniedException` → **۴۰۳** (تصمیم صریح صاحب پروژه؛ ۴۰۴ رد شد). ردیف با `VAHEDCODE` خالی = رکورد سراسری و برای همه مجاز می‌ماند. 🔴 **همچنان باز: ۳۷ نام در `NotYetOwnershipScoped`** — هر کدام یعنی با دانستن `id` می‌شود رکورد واحد دیگری را خواند/ویرایش/حذف کرد. تست‌های convention فاز ۱۹ **معکوس شدند**: حالا هر `Get…ByIdQuery` و هر `Delete…Command` *باید* `IVahedScoped` باشد. ⚠️ ۲۱ مورد **دائماً** معاف‌اند (۸ جدول بدون ستون `VAHEDCODE` + `VahedInfo` + `PersonAction`). **مسدودکنندهٔ استقرار.** | فاز ۷–۱۶؛ نیمه‌حل در ۱۹؛ در حال حل در ۳۲ |
