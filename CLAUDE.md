...
- [ ] رفع باگ‌های `bool?`→enum — فهرست انباشته حالا **~۱۷ ستون در مسیر نوشتن فعال** است (از فازهای ۱۲/۱۳/۱۴/۱۵). جزئیات: `docs/open-decisions.md`.
...
| # | ریسک 🔴 | کشف‌شده در |
|---|---|---|
| ۱ | **IDOR — هیچ authorization در سطح رکورد وجود ندارد.** هر کاربر احراز‌شده می‌تواند هر رکوردی از هر واحد سازمانی را بخواند/بسازد/ویرایش/حذف کند؛ `VAHEDCODE` سمت سرور اعمال نمی‌شود. با هر فاز سطح حمله بزرگ‌تر شده (حالا همهٔ Endpointهای فازهای ۷–۱۵، شامل **شماره حساب/شبا/کارت بانکی** و گردش بانکی). **مسدودکنندهٔ استقرار.** | فاز ۷/۸/۱۰/۱۳/۱۴/۱۵ |
| ۲ | **باگ `bool?`→enum روی ستون‌های چندمقداری `NUMBER(1)`** — ~۱۷ ستون در مسیر نوشتن فعال، شامل موارد تأییدشدهٔ `TB_ACCOUNTCODE.TYPECODE`، `TB_VOUCHERSHEAD.DOCLIFE`، `TB_WHITEANDBLACKLIST.STATE`، و **قطعی‌ترین مورد `TB_ELAMHEAD.ELAMHDRAMAD_TYPE`** (کامنت اوراکل صراحتاً سه‌مقداری). | فاز ۱۲/۱۳/۱۴/۱۵ |
...
| ۱۵ | **حذف `ElamHead`/`CheckBook` هیچ cascade ای به فرزندانشان ندارد** (`TB_ELAMDETAIL`, `TB_CHECK`) — همان وضعیت سند پیش از فاز ۹. مرز Aggregate `ElamHead`/`ElamDetail` هنوز تصمیم‌گیری نشده. | فاز ۱۵ |
