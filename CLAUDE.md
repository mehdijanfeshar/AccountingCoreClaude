...
- [ ] 🚧 **فاز ۴۳ — «گزارش متقاطع» (ناتمام؛ بک‌اند تمام، فرانت شروع نشده).** به درخواست صریح صاحب پروژه از روی ویدیوی نرم‌افزار برهان سیستم: یک گزارش **جدید**، بدون دست‌زدن به «گزارش ماتریسی» موجود. سطر یک بُعد، ستون بُعدی دیگر، و بدهکار/بستانکار در هر تقاطع — یعنی pivot واقعی، برخلاف ماتریسی فعلی که روی *یک* سطح گروه‌بندی می‌کند و خروجی‌اش لیست است. **یافتهٔ کلیدی: قانون کاری #۲ با ساختار برآورده می‌شود و استثنای جدیدی لازم نیست** — `VW_CONSOLIDATE_REPORT` از قبل سلسله‌مراتب حساب و هر ۷ سطح تفصیلی را روی هر سطر تخت کرده، پس کل pivot یک `GROUP BY` روی read model است بدون هیچ join و بدون SQL خام (روی اوراکل زنده: ۱۳۸ms). `GET /api/reports/cross-tab`. **۲۹۹۰ تست سبز.** commit `02fdb26`.
  > ⬅️ **برای ادامه، اول `docs/cross-tab-report-handoff.md` را بخوان.** قرارداد Endpoint، هر هشت تصمیم طراحی، کارهای باقی‌ماندهٔ فرانت، و تلهٔ sticky column در RTL همه آنجاست. پس از تکمیل، آن فایل حذف شود و خلاصه‌اش به `docs/phase-log.md` برود.
