...
public sealed class UpdateAccountCodeInterfaceCommandHandlerTests
{
    private static UpdateAccountCodeInterfaceCommand ValidCommand(Guid id) => new(
        Id: id,
        Type: false,
    ...
    public async Task Handle_ExistingNonDeletedRecord_WritesAllWritableFieldsFromCommand()
    {
        ...
        var handler = new UpdateAccountCodeInterfaceCommandHandler(repository.Object, unitOfWork.Object, currentUser.Object);
        var command = ValidCommand(id);

        await handler.Handle(command, CancellationToken.None);

        Assert.Equal(command.Type, entity.TYPE);
        Assert.Equal(command.AccountCodeId, entity.ACCOUNTCODEID);
    }
