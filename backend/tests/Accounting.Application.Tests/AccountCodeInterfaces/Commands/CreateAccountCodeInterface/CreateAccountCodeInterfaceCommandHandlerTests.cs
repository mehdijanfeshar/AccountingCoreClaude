...
public sealed class CreateAccountCodeInterfaceCommandHandlerTests
{
    ...
    public async Task Handle_MapsCommandFieldsOntoStagedEntity()
    {
        ...
        TB_ACCOUNTCODE_INTERFACE? staged = null;
        repository
            .Setup(r => r.AddAsync(It.IsAny<TB_ACCOUNTCODE_INTERFACE>(), It.IsAny<CancellationToken>()))
            .Callback<TB_ACCOUNTCODE_INTERFACE, CancellationToken>((entity, _) => staged = entity)
            .Returns(Task.CompletedTask);

        var handler = new CreateAccountCodeInterfaceCommandHandler(repository.Object, unitOfWork.Object, currentUser.Object);
        var command = ValidCommand();

        await handler.Handle(command, CancellationToken.None);

        Assert.NotNull(staged);
        Assert.Equal(command.Type, staged!.TYPE);
        Assert.Equal(command.AccountCodeId, staged.ACCOUNTCODEID);
    }
    ...
    public async Task Handle_CallsAddAsyncExactlyOnceAndSaveChangesExactlyOnce()
    {
        ...
        repository.Verify(
            r => r.AddAsync(It.IsAny<TB_ACCOUNTCODE_INTERFACE>(), It.IsAny<CancellationToken>()),
            Times.Once);
    ...
    public async Task Handle_PropagatesCancellationTokenToBothDependencies()
    {
        ...
        repository.Verify(r => r.AddAsync(It.IsAny<TB_ACCOUNTCODE_INTERFACE>(), token), Times.Once);
