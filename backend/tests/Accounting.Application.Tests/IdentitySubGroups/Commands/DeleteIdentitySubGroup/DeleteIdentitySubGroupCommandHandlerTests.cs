...
public sealed class DeleteIdentitySubGroupCommandHandlerTests
{
    ...
    {
        var repositoryMethodNames = typeof(IIdentitySubGroupRepository)
            .GetMethods()
            .Select(m => m.Name)
            .ToArray();
    ...
    public async Task Handle_RecordDoesNotExist_ThrowsNotFoundException_AndNeverCallsSaveChanges()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(
            () => handler.Handle(new DeleteIdentitySubGroupCommand(id), CancellationToken.None));
