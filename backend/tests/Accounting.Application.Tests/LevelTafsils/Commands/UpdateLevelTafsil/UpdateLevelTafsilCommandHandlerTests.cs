...
public sealed class UpdateLevelTafsilCommandHandlerTests
{
    private static UpdateLevelTafsilCommand ValidCommand(Guid id) => new(
        Id: id,
        LevelCode: "02",
        LevelName: "سطح دو");

    private static TB_LEVEL_TAFSIL ExistingEntity(Guid id, bool isDeleted = false) => new()
    {
        ID = id,
        LEVEL_CODE = "01",
        LEVEL_NAME = "سطح یک",
    ...
    public async Task Handle_ExistingNonDeletedRecord_WritesAllWritableFieldsFromCommand()
    {
        ...
        var handler = new UpdateLevelTafsilCommandHandler(repository.Object, unitOfWork.Object, currentUser.Object);
        var command = ValidCommand(id);

        await handler.Handle(command, CancellationToken.None);

        Assert.Equal(command.LevelCode, entity.LEVEL_CODE);
        Assert.Equal(command.LevelName, entity.LEVEL_NAME);
    }
    ...
    public async Task Handle_RecordDoesNotExist_ThrowsNotFoundException_AndNeverCallsSaveChanges()
    {
        var id = Guid.NewGuid();
        var repository = new Mock<ILevelTafsilRepository>();
        repository.Setup(r => r.GetForUpdateAsync(id, It.IsAny<CancellationToken>())).ReturnsAsync((TB_LEVEL_TAFSIL?)null);
        var unitOfWork = new Mock<IUnitOfWork>();
        var currentUser = CurrentUserMock();
