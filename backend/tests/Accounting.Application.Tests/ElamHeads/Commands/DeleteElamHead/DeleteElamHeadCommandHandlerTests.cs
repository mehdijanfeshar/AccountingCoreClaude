...
public sealed class DeleteElamHeadCommandHandlerTests
{
    ...
    public async Task Handle_ExistingNonDeletedRecord_SetsIsDeletedTrue_StampsAuditFields_AndSavesOnce()
    {
        ...
        var handler = new DeleteElamHeadCommandHandler(repository.Object, unitOfWork.Object, currentUser.Object);

        await handler.Handle(new DeleteElamHeadCommand(id), CancellationToken.None);
    ...
    {
        var repositoryMethodNames = typeof(IElamHeadRepository)
            .GetMethods()
            .Select(m => m.Name)
            .ToArray();
    ...
    public async Task Handle_RecordDoesNotExist_ThrowsNotFoundException_AndNeverCallsSaveChanges()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(
            () => handler.Handle(new DeleteElamHeadCommand(id), CancellationToken.None));
    ...
    public async Task Handle_PropagatesCancellationTokenToRepositoryAndUnitOfWork()
    {
        ...
        await handler.Handle(new DeleteElamHeadCommand(id), token);
