...
{
    private static UpdateBankAccountCommand ValidCommand(Guid id) => new(
        Id: id,
    ...
    public async Task Handle_ExistingNonDeletedRecord_WritesAllWritableFieldsFromCommand()
    {
        ...
        Assert.Equal(command.AccountHolder, entity.ACCOUNTHOLDER);
        Assert.Equal(command.CardNumber, entity.CARDNUMBER);
        Assert.Equal(command.ShebaNumber, entity.SHEBANUMBER);
        Assert.Equal(command.FirstAmount, entity.FIRSTAMOUNT);
        Assert.Equal(command.BankId, entity.BANK_ID);
        Assert.Equal(command.BranchId, entity.BRANCH_ID);
        Assert.Equal(command.AccountTypeId, entity.ACCOUNTTYPE_ID);
        ...
        Assert.Equal(command.AccountOpeningDate, entity.ACCOUNTOPENINGDATE);
    }
