...
public sealed class CreateCheckBookCommandHandlerTests
{
    ...
    public async Task Handle_MapsCommandFieldsOntoStagedEntity()
    {
        ...
        Assert.Equal(command.CheckBookTitle, staged.CHECKBOOK_TITLE);
        Assert.Equal(command.CheckBookDate, staged.CHECKBOOK_DATE);
        Assert.Equal(command.FromCheckNumber, staged.FROMCHECKNUMBER);
        Assert.Equal(command.ToCheckNumber, staged.TOCHECKNUMBER);
        Assert.Equal(command.CheckTypeId, staged.CHECKTYPE_ID);
        Assert.Equal(command.VahedCode, staged.VAHEDCODE);
        Assert.Equal(command.CheckBookType, staged.CHECKBOOK_TYPE);
        Assert.Equal(command.Serial, staged.SERIAL);
    }
    ...
    public async Task Handle_CallsAddAsyncExactlyOnceAndSaveChangesExactlyOnce()
    {
        ...
        repository.Verify(
            r => r.AddAsync(It.IsAny<TB_CHECKBOOK>(), It.IsAny<CancellationToken>()),
            Times.Once);
