...
public sealed class GetTrialBalance6QueryHandlerTests
{
    ...
    private static GetTrialBalance6Query Query() => new(
        Year: "1405",
        FromDate: "14050101",
        ToDate: "14051230",
        VahedCode: "0001",
    ...
    /// FirstDebtor/FirstCreditor must be netted (one-sided), matching standard trial-balance
    /// convention, even though the reference project's own formula is raw subtraction.
    /// </summary>
    [Fact]
    public async Task Handle_MapsFirstDebtorAndFirstCreditor_AsNettedOpeningBalance()
    {
        ...
        // Both opening sides non-zero on input, to prove the handler nets them down to one side.
        readRepository
    ...
    [Fact]
    public async Task Handle_CreditorHeavyOpening_FirstCreditorIsPositiveDifference_FirstDebtorIsZero()
    {
        var readRepository = new Mock<ITrialBalanceReadRepository>();
        readRepository
            .Setup(r => r.GetAggregatesAsync(
        ...
        Assert.Equal(0m, row.FirstDebtor);
        Assert.Equal(400m, row.FirstCreditor);
    }
    ...
    {
        var parameterTypes = typeof(GetTrialBalance6QueryHandler)
            .GetConstructors()
