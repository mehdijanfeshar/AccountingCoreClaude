...
public sealed class DeleteIdentityGroupCommandHandlerTests
{
    private static TB_IDENTITYGROUP ExistingEntity(Guid id, bool isDeleted = false) => new()
    {
        ID = id,
        IDENTITYGROUPS_DESC = "desc",
    ...
    public async Task Handle_ExistingNonDeletedRecord_SetsIsDeletedTrue_StampsAuditFields_AndSavesOnce()
    {
        ...
        var handler = new DeleteIdentityGroupCommandHandler(repository.Object, unitOfWork.Object, currentUser.Object);

        await handler.Handle(new DeleteIdentityGroupCommand(id), CancellationToken.None);
    ...
    {
        var repositoryMethodNames = typeof(IIdentityGroupRepository)
            .GetMethods()
            .Select(m => m.Name)
            .ToArray();
    ...
    public async Task Handle_RecordDoesNotExist_ThrowsNotFoundException_AndNeverCallsSaveChanges()
    {
        var id = Guid.NewGuid();
        var repository = new Mock<IIdentityGroupRepository>();
        repository.Setup(r => r.GetForUpdateAsync(id, It.IsAny<string>(), It.IsAny<CancellationToken>())).ReturnsAsync((TB_IDENTITYGROUP?)null);
        var unitOfWork = new Mock<IUnitOfWork>();
        var currentUser = CurrentUserMock();
        ...
        await Assert.ThrowsAsync<NotFoundException>(
            () => handler.Handle(new DeleteIdentityGroupCommand(id), CancellationToken.None));
    ...
    public async Task Handle_PropagatesCancellationTokenToRepositoryAndUnitOfWork()
    {
        ...
        await handler.Handle(new DeleteIdentityGroupCommand(id), token);
