...
public sealed class DeleteRevolvingFundCommandHandlerTests
{
    ...
    public async Task Handle_ExistingNonDeletedRecord_SetsIsDeletedTrue_StampsAuditFields_AndSavesOnce()
    {
        ...
        var handler = new DeleteRevolvingFundCommandHandler(repository.Object, unitOfWork.Object, currentUser.Object);

        await handler.Handle(new DeleteRevolvingFundCommand(id), CancellationToken.None);
    ...
    {
        var repositoryMethodNames = typeof(IRevolvingFundRepository)
            .GetMethods()
            .Select(m => m.Name)
            .ToArray();
    ...
    public async Task Handle_RecordDoesNotExist_ThrowsNotFoundException_AndNeverCallsSaveChanges()
    {
        var id = Guid.NewGuid();
        var repository = new Mock<IRevolvingFundRepository>();
        repository.Setup(r => r.GetForUpdateAsync(id, It.IsAny<string>(), It.IsAny<CancellationToken>())).ReturnsAsync((TB_REVOLVING_FUND?)null);
        var unitOfWork = new Mock<IUnitOfWork>();
        var currentUser = CurrentUserMock();
        ...
        await Assert.ThrowsAsync<NotFoundException>(
            () => handler.Handle(new DeleteRevolvingFundCommand(id), CancellationToken.None));
    ...
    public async Task Handle_PropagatesCancellationTokenToRepositoryAndUnitOfWork()
    {
        ...
        await handler.Handle(new DeleteRevolvingFundCommand(id), token);
