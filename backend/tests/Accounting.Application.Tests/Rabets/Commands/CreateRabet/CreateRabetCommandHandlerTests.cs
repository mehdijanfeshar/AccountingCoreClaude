...
public sealed class CreateRabetCommandHandlerTests
{
    private static CreateRabetCommand ValidCommand() => new(
    ...
    public async Task Handle_MapsCommandFieldsOntoStagedEntity()
    {
        ...
        TB_RABET? staged = null;
        repository
            .Setup(r => r.AddAsync(It.IsAny<TB_RABET>(), It.IsAny<CancellationToken>()))
            .Callback<TB_RABET, CancellationToken>((entity, _) => staged = entity)
            .Returns(Task.CompletedTask);

        var handler = new CreateRabetCommandHandler(repository.Object, unitOfWork.Object, currentUser.Object);
        var command = ValidCommand();

        await handler.Handle(command, CancellationToken.None);

        Assert.NotNull(staged);
        Assert.Equal(command.RabetTypeId, staged!.RABETTYPE_ID);
    ...
    public async Task Handle_CallsAddAsyncExactlyOnceAndSaveChangesExactlyOnce()
    {
        ...
        repository.Verify(
            r => r.AddAsync(It.IsAny<TB_RABET>(), It.IsAny<CancellationToken>()),
            Times.Once);
    ...
    public async Task Handle_PropagatesCancellationTokenToBothDependencies()
    {
        ...
        repository.Verify(r => r.AddAsync(It.IsAny<TB_RABET>(), token), Times.Once);
