...
public sealed class DeleteRabetCommandHandlerTests
{
    private static TB_RABET ExistingEntity(Guid id, bool? isDeleted = false) => new()
    {
        ID = id,
        RABETTYPE_ID = Guid.NewGuid(),
    ...
    public async Task Handle_ExistingNonDeletedRecord_SetsIsDeletedTrue_StampsAuditFields_AndSavesOnce()
    {
        ...
        var repository = new Mock<IRabetRepository>();
        ...
        var handler = new DeleteRabetCommandHandler(repository.Object, unitOfWork.Object, currentUser.Object);

        await handler.Handle(new DeleteRabetCommand(id), CancellationToken.None);
    ...
    {
        var repositoryMethodNames = typeof(IRabetRepository)
            .GetMethods()
            .Select(m => m.Name)
            .ToArray();
    ...
    public async Task Handle_RecordDoesNotExist_ThrowsNotFoundException_AndNeverCallsSaveChanges()
    {
        var id = Guid.NewGuid();
        var repository = new Mock<IRabetRepository>();
        repository.Setup(r => r.GetForUpdateAsync(id, It.IsAny<CancellationToken>())).ReturnsAsync((TB_RABET?)null);
        var unitOfWork = new Mock<IUnitOfWork>();
        var currentUser = CurrentUserMock();
        ...
        await Assert.ThrowsAsync<NotFoundException>(
            () => handler.Handle(new DeleteRabetCommand(id), CancellationToken.None));
    ...
    public async Task Handle_PropagatesCancellationTokenToRepositoryAndUnitOfWork()
    {
        ...
        await handler.Handle(new DeleteRabetCommand(id), token);
