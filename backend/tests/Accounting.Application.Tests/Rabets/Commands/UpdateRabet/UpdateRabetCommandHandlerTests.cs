...
public sealed class UpdateRabetCommandHandlerTests
{
    private static UpdateRabetCommand ValidCommand(Guid id) => new(
        Id: id,
    ...
    public async Task Handle_ExistingNonDeletedRecord_WritesAllWritableFieldsFromCommand()
    {
        ...
        var handler = new UpdateRabetCommandHandler(repository.Object, unitOfWork.Object, currentUser.Object);
        var command = ValidCommand(id);

        await handler.Handle(command, CancellationToken.None);

        Assert.Equal(command.RabetTypeId, entity.RABETTYPE_ID);
