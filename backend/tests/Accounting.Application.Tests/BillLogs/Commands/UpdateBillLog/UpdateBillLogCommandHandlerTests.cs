...
public sealed class UpdateBillLogCommandHandlerTests
{
    private static UpdateBillLogCommand ValidCommand(Guid id) => new(
        Id: id,
        LogDesc: "updated desc",
        LogDate: "14030202",
        Year: "1403")
    {
        VahedCode = "0200",
    ...
    private static TB_BILL_LOG ExistingEntity(Guid id, bool isDeleted = false) => new()
    {
        ID = id,
        LOG_DESC = "old desc",
        LOG_DATE = "14030101",
    ...
    public async Task Handle_ExistingNonDeletedRecord_WritesAllWritableFieldsFromCommand()
    {
        ...
        var handler = new UpdateBillLogCommandHandler(repository.Object, unitOfWork.Object, currentUser.Object);
        var command = ValidCommand(id);

        await handler.Handle(command, CancellationToken.None);

        Assert.Equal(command.LogDesc, entity.LOG_DESC);
        Assert.Equal(command.LogDate, entity.LOG_DATE);
    ...
    public async Task Handle_RecordDoesNotExist_ThrowsNotFoundException_AndNeverCallsSaveChanges()
    {
        var id = Guid.NewGuid();
        var repository = new Mock<IBillLogRepository>();
        repository.Setup(r => r.GetForUpdateAsync(id, It.IsAny<string>(), It.IsAny<CancellationToken>())).ReturnsAsync((TB_BILL_LOG?)null);
        var unitOfWork = new Mock<IUnitOfWork>();
        var currentUser = CurrentUserMock();
    ...
    public async Task Handle_ThroughVahedScopeBehavior_ClientSuppliedVahedCodeIsDiscarded_ServerValueIsWritten()
    {
        ...
        var handler = new UpdateBillLogCommandHandler(repository.Object, unitOfWork.Object, currentUser.Object);
        var behavior = new VahedScopeBehavior<UpdateBillLogCommand, Unit>(TestUnitScope.ResolverFor(currentUser.Object));
