...
public sealed class DeleteBillLogCommandHandlerTests
{
    ...
    public async Task Handle_ExistingNonDeletedRecord_SetsIsDeletedTrue_StampsAuditFields_AndSavesOnce()
    {
        ...
        var handler = new DeleteBillLogCommandHandler(repository.Object, unitOfWork.Object, currentUser.Object);

        await handler.Handle(new DeleteBillLogCommand(id), CancellationToken.None);
    ...
    {
        var repositoryMethodNames = typeof(IBillLogRepository)
            .GetMethods()
            .Select(m => m.Name)
            .ToArray();
    ...
    public async Task Handle_RecordDoesNotExist_ThrowsNotFoundException_AndNeverCallsSaveChanges()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(
            () => handler.Handle(new DeleteBillLogCommand(id), CancellationToken.None));
    ...
    public async Task Handle_PropagatesCancellationTokenToRepositoryAndUnitOfWork()
    {
        ...
        await handler.Handle(new DeleteBillLogCommand(id), token);
