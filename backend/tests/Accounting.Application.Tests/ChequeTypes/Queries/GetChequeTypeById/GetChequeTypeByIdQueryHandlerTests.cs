...
public sealed class GetChequeTypeByIdQueryHandlerTests
{
    ...
    public async Task Handle_ExistingId_ReturnsRepositoryDto()
    {
        ...
        var result = await handler.Handle(new GetChequeTypeByIdQuery(id), CancellationToken.None);
    ...
    {
        var parameterTypes = typeof(GetChequeTypeByIdQueryHandler)
            .GetConstructors()
