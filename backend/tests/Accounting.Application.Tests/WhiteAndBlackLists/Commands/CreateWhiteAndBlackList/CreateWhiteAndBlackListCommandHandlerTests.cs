...
public sealed class CreateWhiteAndBlackListCommandHandlerTests
{
    ...
    public async Task Handle_MapsCommandFieldsOntoStagedEntity()
    {
        ...
        TB_WHITEANDBLACKLIST? staged = null;
        repository
            .Setup(r => r.AddAsync(It.IsAny<TB_WHITEANDBLACKLIST>(), It.IsAny<CancellationToken>()))
            .Callback<TB_WHITEANDBLACKLIST, CancellationToken>((entity, _) => staged = entity)
            .Returns(Task.CompletedTask);

        var handler = new CreateWhiteAndBlackListCommandHandler(repository.Object, unitOfWork.Object, currentUser.Object);
        ...
        Assert.Equal(command.State, staged.STATE);
    }
    ...
    public async Task Handle_CallsAddAsyncExactlyOnceAndSaveChangesExactlyOnce()
    {
        ...
        repository.Verify(
            r => r.AddAsync(It.IsAny<TB_WHITEANDBLACKLIST>(), It.IsAny<CancellationToken>()),
            Times.Once);
    ...
    public async Task Handle_PropagatesCancellationTokenToBothDependencies()
    {
        ...
        repository.Verify(r => r.AddAsync(It.IsAny<TB_WHITEANDBLACKLIST>(), token), Times.Once);
