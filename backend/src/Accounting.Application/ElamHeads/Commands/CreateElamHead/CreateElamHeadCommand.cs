...
///
/// ✅ <b>Both instances of the phase-12 <c>bool?</c>-should-be-enum bug on this table were fixed
/// on ۲۰۲۶-۰۹-۲۰</b> (batch 5 of open risk #2). This is a <b>breaking API-contract change</b>:
/// <c>Case</c> and <c>DramadType</c> are now integers, not booleans.
/// <list type="bullet">
/// <item><description><c>Case</c> (<c>ELAMH_CASE</c>) → <see cref="ElamCase"/>. Unambiguous: the
/// Oracle column comment («نوع اعلاميه 1بد 2بس») and the reference project's enum agree
/// exactly — the only column of this family where they do.</description></item>
/// <item><description><c>DramadType</c> (<c>ELAMHDRAMAD_TYPE</c>) → <see cref="DaramElamhType"/>,
/// three values (the third was previously unreachable through this API entirely). ⚠️ The Oracle
/// comment and the reference enum are a full three-way permutation of each other; the reference
/// ordering was chosen. Cost of being wrong is labels only, never stored values — see that
/// enum's XML doc and <c>docs/open-decisions.md</c>.</description></item>
/// </list>
///
...
/// <param name="DramadType">ELAMHDRAMAD_TYPE column (optional <see cref="DaramElamhType"/> — three values; ⚠️ label ordering follows the reference project, not the Oracle comment).</param>
