...
/// <summary>
/// Runs the 6-column trial balance (تراز ۶ ستونی) aggregate query and returns one
/// <see cref="TrialBalance6RowDto"/> per distinct account code at <see cref="Level"/>. READ-ONLY —
...
/// <see cref="GetTrialBalance6QueryValidator"/> XML doc for why this is mandatory.</param>
...
public sealed record GetTrialBalance6Query(
    string Year,
    string? FromDate,
    string? ToDate,
    TrialBalanceLevel Level,
    int? DocLife) : IRequest<IReadOnlyList<TrialBalance6RowDto>>, IVahedScopedQuery
{
    /// <summary>
