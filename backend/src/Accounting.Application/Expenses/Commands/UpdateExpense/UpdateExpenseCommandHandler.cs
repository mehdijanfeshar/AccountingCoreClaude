...
/// <summary>
/// Loads the existing <see cref="Accounting.Domain.Entity.TB_EXPENCE"/> row via
/// <see cref="IExpenseRepository.GetForUpdateAsync"/> (change-tracked), overwrites every writable
...
/// soft-deleted. <c>ISDELETED</c> is <c>bool?</c> on this table, so both <see langword="false"/>
/// and <see langword="null"/> are treated as "not deleted" — only an explicit
...
public sealed class UpdateExpenseCommandHandler : IRequestHandler<UpdateExpenseCommand>
{
    ...
    public async Task Handle(UpdateExpenseCommand request, CancellationToken cancellationToken)
    {
        ...
        entity.EXPENCECODE = request.ExpenseCode;
        entity.EXPENCENAME = request.ExpenseName;
        entity.DESCRIPTION = request.Description;
        entity.DEFAULTAMOUNT = request.DefaultAmount;
        entity.EXPENCEGROUP_ID = request.ExpenseGroupId;
    ...
    /// <summary>
    /// Brings <c>TB_EXPENCE_LINK_TAFSILI</c> in line with the request's replacement set, matching existing
    ...
    /// </summary>
    private async Task ReconcileTafsiliLinksAsync(UpdateExpenseCommand request, CancellationToken cancellationToken)
    {
        var requested = request.TafsiliLinks ?? Array.Empty<ExpenseTafsiliLinkInput>();
        var existing = await _expenseRepository.GetActiveTafsiliLinksAsync(request.Id, cancellationToken);
