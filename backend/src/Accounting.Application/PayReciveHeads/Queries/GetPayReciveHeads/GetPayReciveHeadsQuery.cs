...
/// Only non-deleted rows (<c>ISDELETED == false</c>) are included, and — via
...
/// <c>PayReciveHeadReadRepository.GetPagedAsync</c> for why the filter is unconditional).
/// </summary>
/// <param name="PageNumber">1-based page number.</param>
/// <param name="PageSize">Page size, capped by <see cref="GetPayReciveHeadsQueryValidator.MaxPageSize"/>.</param>
public sealed record GetPayReciveHeadsQuery(int PageNumber, int PageSize)
    : IRequest<PagedResult<PayReciveHeadDto>>, IVahedScopedQuery
{
    /// <summary>
