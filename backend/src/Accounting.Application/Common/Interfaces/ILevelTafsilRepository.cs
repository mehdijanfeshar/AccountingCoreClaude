...
/// <summary>
/// Write-side repository for <see cref="TB_LEVEL_TAFSIL"/>. Only stages changes — it does NOT
/// call SaveChanges; the handler owns the transaction boundary via <see cref="IUnitOfWork"/>.
/// </summary>
public interface ILevelTafsilRepository
{
    Task AddAsync(TB_LEVEL_TAFSIL levelTafsil, CancellationToken cancellationToken = default);

    /// <summary>
    /// Loads a single <see cref="TB_LEVEL_TAFSIL"/> by <c>ID</c> as a change-tracked entity
