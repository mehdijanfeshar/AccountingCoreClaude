...
/// <summary>
/// Write-side repository for <see cref="TB_IDENTITYGROUP"/>. Only stages changes — it does NOT
/// call SaveChanges; the handler owns the transaction boundary via <see cref="IUnitOfWork"/>.
/// </summary>
public interface IIdentityGroupRepository
{
    Task AddAsync(TB_IDENTITYGROUP identityGroup, CancellationToken cancellationToken = default);

    /// <summary>
    /// Loads a single <see cref="TB_IDENTITYGROUP"/> by <c>ID</c> as a change-tracked entity
    ...
    /// </summary>
    Task<TB_IDENTITYGROUP?> GetForUpdateAsync(Guid id, string vahedCode, CancellationToken cancellationToken = default);
}
