...
/// <summary>
/// Write-side repository for <see cref="TB_CHEQUES_INCORRENT"/>. Only stages changes — it does
/// NOT call SaveChanges; the handler owns the transaction boundary via <see cref="IUnitOfWork"/>.
/// </summary>
public interface IChequesIncorrentRepository
{
    Task AddAsync(TB_CHEQUES_INCORRENT chequesIncorrent, CancellationToken cancellationToken = default);

    /// <summary>
    /// Loads a single <see cref="TB_CHEQUES_INCORRENT"/> by <c>ID</c> as a change-tracked entity
    ...
    /// </summary>
    Task<TB_CHEQUES_INCORRENT?> GetForUpdateAsync(Guid id, string vahedCode, CancellationToken cancellationToken = default);
}
