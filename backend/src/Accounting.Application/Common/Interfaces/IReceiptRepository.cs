...
/// <summary>
/// Write-side repository for <see cref="TB_RECEIP"/>. Only stages changes — it does NOT call
/// SaveChanges; the handler owns the transaction boundary via <see cref="IUnitOfWork"/>.
/// </summary>
public interface IReceiptRepository
{
    Task AddAsync(TB_RECEIP receipt, CancellationToken cancellationToken = default);

    /// <summary>
    /// Loads a single <see cref="TB_RECEIP"/> by <c>ID</c> as a change-tracked entity
    ...
    /// </summary>
    Task<TB_RECEIP?> GetForUpdateAsync(Guid id, string vahedCode, CancellationToken cancellationToken = default);
}
