...
/// <summary>
/// Write-side repository for <see cref="TB_RABET"/>. Only stages changes — it does NOT call
/// SaveChanges; the handler owns the transaction boundary via <see cref="IUnitOfWork"/>.
/// </summary>
public interface IRabetRepository
{
    Task AddAsync(TB_RABET rabet, CancellationToken cancellationToken = default);

    /// <summary>
    /// Loads a single <see cref="TB_RABET"/> by <c>ID</c> as a change-tracked entity
    ...
    /// </summary>
    Task<TB_RABET?> GetForUpdateAsync(Guid id, CancellationToken cancellationToken = default);
}
