...
/// <summary>
/// Write-side repository for <see cref="TB_BANKCARTDETAIL"/>. Only stages changes — it does NOT
/// call SaveChanges; the handler owns the transaction boundary via <see cref="IUnitOfWork"/>.
/// </summary>
public interface IBankCartDetailRepository
{
    Task AddAsync(TB_BANKCARTDETAIL bankCartDetail, CancellationToken cancellationToken = default);

    /// <summary>
    /// Loads a single <see cref="TB_BANKCARTDETAIL"/> by <c>ID</c> as a change-tracked entity
    ...
    /// </summary>
    Task<TB_BANKCARTDETAIL?> GetForUpdateAsync(Guid id, string vahedCode, CancellationToken cancellationToken = default);
}
