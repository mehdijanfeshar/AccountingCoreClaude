...
/// <summary>
/// Write-side repository for <see cref="TB_WHITELIST"/>. Only stages changes — it does NOT
/// call SaveChanges; the handler owns the transaction boundary via <see cref="IUnitOfWork"/>.
/// </summary>
public interface IWhiteListRepository
{
    Task AddAsync(TB_WHITELIST whiteList, CancellationToken cancellationToken = default);

    /// <summary>
    /// Loads a single <see cref="TB_WHITELIST"/> by <c>ID</c> as a change-tracked entity
    ...
    /// </summary>
    Task<TB_WHITELIST?> GetForUpdateAsync(Guid id, CancellationToken cancellationToken = default);
}
