...
/// <summary>
/// Write-side repository for <see cref="TB_CHECK_TYPE"/>. Only stages changes — it does NOT
...
{
    Task AddAsync(TB_CHECK_TYPE chequeType, CancellationToken cancellationToken = default);

    /// <summary>
    /// Loads a single <see cref="TB_CHECK_TYPE"/> by <c>ID</c> as a change-tracked entity
    ...
    /// </summary>
    Task<TB_CHECK_TYPE?> GetForUpdateAsync(Guid id, string vahedCode, CancellationToken cancellationToken = default);
}
