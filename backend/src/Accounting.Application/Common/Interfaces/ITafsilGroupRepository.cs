...
/// <summary>
/// Write-side repository for <see cref="TB_TAFSIL_GROUP"/>. Only stages changes — it does NOT
/// call SaveChanges; the handler owns the transaction boundary via <see cref="IUnitOfWork"/>.
/// </summary>
public interface ITafsilGroupRepository
{
    Task AddAsync(TB_TAFSIL_GROUP tafsilGroup, CancellationToken cancellationToken = default);

    /// <summary>
    /// Loads a single <see cref="TB_TAFSIL_GROUP"/> by <c>ID</c> as a change-tracked entity
    ...
    /// </summary>
    Task<TB_TAFSIL_GROUP?> GetForUpdateAsync(Guid id, CancellationToken cancellationToken = default);
}
