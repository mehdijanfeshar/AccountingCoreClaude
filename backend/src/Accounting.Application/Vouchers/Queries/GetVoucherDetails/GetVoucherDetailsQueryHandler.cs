...
/// <see cref="GetVoucherDetailsQuery.VahedCode"/> through at face value — by the time this
/// handler runs, <c>VahedScopeBehavior</c> has already overwritten it with the authenticated
/// caller's own unit code, so this handler must forward exactly that value, never derive its own.
/// </summary>
