...
public sealed class ChequesIncorrentReadRepository : IChequesIncorrentReadRepository
{
    ...
    public async Task<ChequesIncorrentDto?> GetByIdAsync(
        Guid id,
        string vahedCode,
        CancellationToken cancellationToken = default)
    {
        ...
        var ownerVahedCode = await _dbContext.TB_CHEQUES_INCORRENTs
            .AsNoTracking()
            .Where(c => c.ID == id)
            .Select(c => c.VAHEDCODE)
            .FirstOrDefaultAsync(cancellationToken);

        VahedOwnership.EnsureOwned(ownerVahedCode, vahedCode, id, "ChequesIncorrent");
