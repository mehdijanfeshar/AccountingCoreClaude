...
public sealed class IdentityGroupReadRepository : IIdentityGroupReadRepository
{
    ...
    public async Task<IdentityGroupDto?> GetByIdAsync(
        Guid id,
        string vahedCode,
        CancellationToken cancellationToken = default)
    {
        ...
        var ownerVahedCode = await _dbContext.TB_IDENTITYGROUPs
            .AsNoTracking()
            .Where(g => g.ID == id)
            .Select(g => g.VAHEDCODE)
            .FirstOrDefaultAsync(cancellationToken);

        VahedOwnership.EnsureOwned(ownerVahedCode, vahedCode, id, "IdentityGroup");
