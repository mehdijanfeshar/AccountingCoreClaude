...
public sealed class ElamHeadReadRepository : IElamHeadReadRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // the row's unit — per explicit project-owner decision. TB_ELAMHEAD.VAHEDCODE is itself
        // nullable in Legacy, so this matters more here than on the other three entities in this
        // batch (all of which have a non-nullable VAHEDCODE column).
    ...
    public async Task<ElamHeadDto?> GetByIdAsync(
        Guid id,
        string vahedCode,
        CancellationToken cancellationToken = default)
    {
        ...
        var ownerVahedCode = await _dbContext.TB_ELAMHEADs
            .AsNoTracking()
            .Where(e => e.ID == id)
            .Select(e => e.VAHEDCODE)
            .FirstOrDefaultAsync(cancellationToken);

        VahedOwnership.EnsureOwned(ownerVahedCode, vahedCode, id, "ElamHead");
