...
public sealed class CheckBookReadRepository : ICheckBookReadRepository
{
    ...
    public async Task<CheckBookDto?> GetByIdAsync(
        Guid id,
        string vahedCode,
        CancellationToken cancellationToken = default)
    {
        ...
        var ownerVahedCode = await _dbContext.TB_CHECKBOOKs
            .AsNoTracking()
            .Where(c => c.ID == id)
            .Select(c => c.VAHEDCODE)
            .FirstOrDefaultAsync(cancellationToken);

        VahedOwnership.EnsureOwned(ownerVahedCode, vahedCode, id, "CheckBook");
