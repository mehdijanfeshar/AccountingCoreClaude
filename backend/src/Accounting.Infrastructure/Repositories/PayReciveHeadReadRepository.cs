...
public sealed class PayReciveHeadReadRepository : IPayReciveHeadReadRepository
{
    ...
    public async Task<PayReciveHeadDto?> GetByIdAsync(
        Guid id,
        string vahedCode,
        CancellationToken cancellationToken = default)
    {
        ...
        var ownerVahedCode = await _dbContext.TB_PAYRECIVHEADs
            .AsNoTracking()
            .Where(e => e.ID == id)
            .Select(e => e.VAHEDCODE)
            .FirstOrDefaultAsync(cancellationToken);

        VahedOwnership.EnsureOwned(ownerVahedCode, vahedCode, id, "PayReciveHead");
