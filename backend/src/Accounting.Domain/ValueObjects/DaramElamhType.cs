...
///
/// ✅ <b>ترتیب برچسب‌ها در ۲۰۲۶-۰۹-۲۰ توسط صاحب پروژه صریحاً تأیید شد</b> (متن enum پروژهٔ مرجع
/// را عیناً برگرداند). بخش زیر به‌عنوان سابقهٔ تصمیم نگه داشته شده، نه ابهام باز.
///
/// این یکی از پنج موردِ §۱۰-۴ سند
...
///
/// <b>هزینهٔ اشتباه فقط برچسب بود، نه داده:</b> مقادیر عددی ذخیره‌شده تغییری نمی‌کنند؛ اگر ترتیب
/// غلط می‌بود، صرفاً UI نام نادرست را کنار عدد درست نشان می‌داد. همین باعث شد پیاده‌سازی منتظر
/// تأیید نماند — ولی تأیید هم گرفته شد (بالا).
/// </summary>
