...
به انتخاب صریح کاربر (گزینهٔ «ج»: Legacy جایگزین کامل مدل Rich)، مدل نوشتن پروژه اکنون Entityهای `Accounting.Domain.Legacy` است و invariantهای زیر **آگاهانه کنار گذاشته شده‌اند**؛ کلاس‌هایی که آن‌ها را enforce می‌کردند `[Obsolete]` شده‌اند:
...
- استفاده از کلاس‌های `[Obsolete]` شدهٔ مدل Rich در کد جدید (به‌جای آن‌ها از `Accounting.Domain.Legacy` استفاده کن)
- حذف فیزیکی کلاس‌های منسوخ یا تست‌های موجود بدون درخواست صریح کاربر
